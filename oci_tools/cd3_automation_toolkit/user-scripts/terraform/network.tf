...
data "oci_core_drg_route_tables" "drg_route_tables" {
    ...
    #Required
    drg_id = merge(module.drgs.*...)[each.value.drg_id]["drg_tf_id"][0]
    filter {
    name = "display_name"
...
module "hub-lpgs" {
  ...
  #Optional
  peer_id = each.value.peer_id != "" ? (length(regexall("##peer_id*", each.value.peer_id)) > 0 ? null : try(merge(module.spoke-lpgs.*...)[each.value.peer_id]["lpg_tf_id"][0],merge(module.exported-lpgs.*...)[each.value.peer_id]["lpg_tf_id"][0],merge(module.peer-lpgs.*...)[each.value.peer_id]["lpg_tf_id"][0],merge(module.none-lpgs.*...)[each.value.peer_id]["lpg_tf_id"][0])) : null
...
module "peer-lpgs" {
  ...
  #Optional
  peer_id = each.value.peer_id != "" ? (length(regexall("##peer_id*", each.value.peer_id)) > 0 ? null : try(merge(module.spoke-lpgs.*...)[each.value.peer_id]["lpg_tf_id"][0],merge(module.exported-lpgs.*...)[each.value.peer_id]["lpg_tf_id"][0],merge(module.none-lpgs.*...)[each.value.peer_id]["lpg_tf_id"][0])) : null
  route_table_id = each.value.route_table_id
  defined_tags  = each.value.defined_tags
  display_name  = each.value.lpg_name != null ? each.value.lpg_name : null
  freeform_tags = each.value.freeform_tags
...
module "drg-route-tables" {
  ...
  display_name  = each.value.display_name != null ? each.value.display_name : null
  import_drg_route_distribution_id = each.value.import_drg_route_distribution_id != null && each.value.import_drg_route_distribution_id  != "" ? (length(regexall("ocid1.drgroutedistribution.oc1*",each.value.import_drg_route_distribution_id)) > 0 ? each.value.import_drg_route_distribution_id : try(merge(module.drg-route-distributions.*...)[each.value.import_drg_route_distribution_id]["drg_route_distribution_tf_id"][0])) : null  #,data.oci_core_drg_route_distributions[each.value.import_drg_route_distribution_id].drg_route_distributions[0].id
...
module "drg-route-rules" {
  ...
  #Required
  drg_route_table_id = length(regexall("ocid1.drgroutetable.oc1*", each.value.drg_route_table_id)) > 0 ? each.value.drg_route_table_id : ((each.value.drg_route_table_id != "" && each.value.drg_route_table_id != null) ? try(merge(module.drg-route-tables.*...)[each.value.drg_route_table_id]["drg_route_table_tf_id"][0],data.oci_core_drg_route_tables.drg_route_tables[each.value.drg_route_table_id].drg_route_tables[0].id) : null) #data.oci_core_drg_route_tables[each.value.drg_route_table_id].drg_route_tables[0].id
...
module "drg-route-distribution-statements" {
  ...
  #Required
  key_name = each.key
  drg_route_distribution_id = length(regexall("ocid1.drgroutedistribution.oc1*",each.value.drg_route_distribution_id)) > 0 ? each.value.drg_route_distribution_id : try(merge(module.drg-route-distributions.*...)[each.value.drg_route_distribution_id]["drg_route_distribution_tf_id"][0],data.oci_core_drg_route_distributions.drg_route_distributions[each.value.drg_route_distribution_id].drg_route_distributions[0].id)
