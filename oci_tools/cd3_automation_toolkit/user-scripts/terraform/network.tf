...
module "drg-route-tables" {
  ...
  display_name                     = each.value.display_name != null ? each.value.display_name : null
  import_drg_route_distribution_id = each.value.import_drg_route_distribution_id != null && each.value.import_drg_route_distribution_id != "" ? (length(regexall("ocid1.drgroutedistribution.oc1*", each.value.import_drg_route_distribution_id)) > 0 ? each.value.import_drg_route_distribution_id : try(merge(module.drg-route-distributions.*...)[each.value.import_drg_route_distribution_id]["drg_route_distribution_tf_id"])) : null #,data.oci_core_drg_route_distributions[each.value.import_drg_route_distribution_id].drg_route_distributions.id
...
module "subnets" {
  ...
  dhcp_options_id            = length(regexall("ocid1.dhcpoptions.oc1*", each.value.dhcp_options_id)) > 0 ? each.value.dhcp_options_id : ((each.value.dhcp_options_id != "" && each.value.dhcp_options_id != null) ? try(merge(module.vcns.*...)[each.value.dhcp_options_id]["vcn_default_dhcp_id"], merge(module.custom-dhcps.*...)[each.value.dhcp_options_id]["custom_dhcp_tf_id"], merge(module.default-dhcps.*...)[each.value.dhcp_options_id]["default_dhcp_tf_id"]) : null)
  route_table_id             = length(regexall("ocid1.routetable.oc1*", each.value.route_table_id)) > 0 ? each.value.route_table_id : ((each.value.route_table_id != "" && each.value.route_table_id != null) ? try(merge(module.vcns.*...)[each.value.route_table_id]["vcn_default_route_table_id"], merge(module.route-tables.*...)[each.value.route_table_id]["route_table_ids"], merge(module.default-route-tables.*...)[each.value.route_table_id]["default_route_table_tf_id"]) : "")
  security_list_ids          = [for sl in each.value.security_list_ids : (length(regexall("ocid1.securitylist.oc1*", sl)) > 0 ? sl : try(merge(module.vcns.*...)[sl]["vcn_default_security_list_id"], merge(module.security-lists.*...)[sl]["seclist_subnet_id"], merge(module.default-security-lists.*...)[sl]["default_seclist_tf_id"]))]
