...
parser = argparse.ArgumentParser(description="Takes in arg mentioning compartment OCID and fetches backups for boot and block vols of all instances in that compartment")
...
for line in fname:
    if not line.startswith('#'):
        if ("Log Retention Days" in line):
            retentionDays = line.split("=")[1].strip()
        ...
        out_file=out_dir+"/backups_"+comp_name+".log"
        yesterday_file=out_file+"_"+yesterday
        if(os.path.exists(out_file)):
            shutil.copy(out_file,yesterday_file)
...
fname.close()
retaindays_ago = today - timedelta(days=retaindays)
file_list=os.listdir(out_dir)

for file in file_list:
    statbuf = os.stat(out_dir+"/"+file)
    modificationDate=datetime.date.fromtimestamp((statbuf.st_mtime))

    if(modificationDate < retaindays_ago):
