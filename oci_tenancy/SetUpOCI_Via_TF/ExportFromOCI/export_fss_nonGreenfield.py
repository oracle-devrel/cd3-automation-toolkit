...
def add_column_data(reg,cname,AD_name,mt_display_name,vplussubnet,mnt_p_ip,mnt_p_hostname,bytes,files,fs_name,einfo_path,sourceCIDR,Access,GID,UID,IDSquash,require_ps_port,fsinfo,values_for_column_fss,mnt_info1):
    for col_header in values_for_column_fss.keys():
        if(col_header == "Region"):
            ...
            values_for_column_fss = commonTools.export_extra_columns(oci_objs, col_header, sheet_dict_instances,values_for_column_fss)
...
def __get_mount_info(cname,compartment_id,reg,availability_domain_name,config):
     ...
     try:    
       mount_info = oci.pagination.list_call_get_all_results(file_system.list_mount_targets,compartment_id=compartment_id, availability_domain=availability_domain_name)
       fss=[]
       for mnt in mount_info.data:
         ...
         export_info = oci.pagination.list_call_get_all_results(file_system.list_exports,export_set_id=export_set_id)
         file_system_info=""
         for einfo in export_info.data:
           ...
           if(elen==0):
             ...
             add_column_data(reg,cname,AD_name,mt_display_name,vplussubnet,mnt_p_ip,mnt_p_hostname,bytes,files,fs_name,einfo_path,sourceCIDR="",Access="",GID="",UID="",IDSquash="",require_ps_port="",fsinfo=file_system_info,values_for_column_fss=values_for_column_fss,mnt_info1=mnt)
           ...
           if(elen==1 or elen>1):
             ...
             add_column_data(reg,cname,AD_name,mt_display_name,vplussubnet,mnt_p_ip,mnt_p_hostname,bytes,files,fs_name,einfo_path,einfo_export_data.export_options[0].source,einfo_export_data.export_options[0].access,einfo_export_data.export_options[0].anonymous_gid,einfo_export_data.export_options[0].anonymous_uid,einfo_export_data.export_options[0].identity_squash,einfo_export_data.export_options[0].require_privileged_source_port,fsinfo=file_system_info,values_for_column_fss=values_for_column_fss,mnt_info1=mnt)
           ...
           if(elen>1):
              for rw in range(1,elen):
                add_column_data("","","","","","","","","","",einfo_path,einfo_export_data.export_options[rw].source,einfo_export_data.export_options[rw].access,einfo_export_data.export_options[rw].anonymous_gid,einfo_export_data.export_options[rw].anonymous_uid,einfo_export_data.export_options[rw].identity_squash,einfo_export_data.export_options[rw].require_privileged_source_port,fsinfo=file_system_info,values_for_column_fss=values_for_column_fss,mnt_info1=mnt)          
