...
def adding_columns_values(region,hostname,ad,fd,vs,publicip,privateip,os,shape,key_name,c_name,bkp_policy_name,nsgs,d_host,instance_data,values_for_column_instances,bc_info):
    for col_header in values_for_column_instances.keys():
        if(col_header == "Region"):
            ...
            values_for_column_instances = commonTools.export_tags(instance_data, col_header, values_for_column_instances)
        else:
            oci_objs=[instance_data,bc_info[0]]
            values_for_column_instances = commonTools.export_extra_columns(oci_objs, col_header, sheet_dict_instances,values_for_column_instances)
...
def find_vnic(ins_id,config):
    ...
    compute=oci.core.ComputeClient(config)
    for comp in all_compartments:
        net=oci.pagination.list_call_get_all_results(compute.list_vnic_attachments,compartment_id=comp,instance_id=ins_id)
        if (len(net.data)):
           return net
            ...
            if (ins.lifecycle_state !='TERMINATED'):
                ...
                for lnic in ins_vnic.data:
                    ...
                    if ('ssh_authorized_keys' in ins.metadata.keys()):
                        ...
                        instance_keys[tf_name]=ins.metadata['ssh_authorized_keys']          # print metadata of the instance 
                        key_name=ins_dname+"_"+str(privateip)
                        key_name = commonTools.check_tf_variable(key_name)
                    else:
                        key_name=reg_name+"-"+key_name
                    ...
                    adding_columns_values(reg_name.title(),ins_dname,AD_name,ins_fd,vs,publicip,privateip,os_dname,ins_shape,key_name,compartment_name,bkp_policy_name,nsg_names,dedicated_host,ins,values_for_column_instances,boot_check.data)
                    #rows.append(new_row)
