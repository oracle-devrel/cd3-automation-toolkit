...
def main():
    ...
    parser.add_argument("--networkCompartment",
                        help="comma seperated Compartments for which to export Identity Objects", required=False)
    ...
    if (os.path.exists(outdir + "/" + ct.home_region + "/tf_import_commands_identity_nonGF.sh")):
        commonTools.backup_file(outdir + "/" + ct.home_region, "tf_import_identity",
        ...
        if (c_id not in comp_ocids_done):
            ...
            for col_header in values_for_column_comps.keys():
                if (col_header == "Region"):
                    ...
                    oci_objs = [c_details]
                    values_for_column_comps = commonTools.export_extra_columns(oci_objs, col_header, sheet_dict_comps,
