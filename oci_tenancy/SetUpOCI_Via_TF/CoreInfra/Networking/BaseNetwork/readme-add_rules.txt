add_routes_to_tf.py and update_seclist_to_tf_v2.0.py are the scripts to add new route rules and security rules respectively to OCI.

add_routes_to_tf.py
--------------------
This script takes in either a csv file or CD3 excel file mentioning new route rules to be added as the input. See add_routes-example.csv under example folder.
Note- This would only add new rules to the route table. It wont check for existing rules.
Usage:

./add_routes_to_tf.py <path to input csv containing rules> <path to routes tf file created earlier using create_all_tf_objects.py>
eg
./add_routes_to_tf.py add-routes.csv routes.tf


add_secrules_to_tf.py
------------------------------
This script takes in either a csv file or CD3 excel file mentioning new sec rules to be added as the input. See update_seclist-example.csv under example folder.


Usage:
./add_secrules_to_tf.py --inputfile <path to vcn-info.properties or CD3 excel file> --outdir <output dir name being used for tf files> --secrulesfile <path to input csv containing rules>
eg
./add_secrules_to_tf.py --inputfile vcn-info.properties --outdir /root/ocswork/terraform_files --secrulesfile update_seclist-adrules.csv
or
./add_secrules_to_tf.py --inputfile CD3-template.properties --outdir /root/ocswork/terraform_files --secrulesfile update_seclist-adrules.csv
