...
def parse_args():
    # Read the arguments
    parser = argparse.ArgumentParser(description="Create Database System Bare Metal terraform file")
...
def create_terraform_database_BM(inputfile, outdir, prefix, config=DEFAULT_LOCATION):
    filename = inputfile
    ...
    df, col_headers = commonTools.read_cd3(filename, "DB_System_BM")
    ...
    for i in df.index:
        ...
        if str(df.loc[i, 'Region']).lower() == 'nan' or \
            ...
            print("\nRegion, Compartment Name, Availability Domain(AD1|AD2|AD3), Oracle DB Software Edition, DB Version, Hostname Prefix, Shape are mandatory fields. Please enter a value and try again.......Exiting!!")
            exit()
    ...
    for reg in ct.all_regions:
        ...
        outfile[reg] = reg_out_dir + "/DBBM.tf"

        if(tfStr[reg]!=''):
            oname[reg]=open(outfile[reg],'w')
            oname[reg].write(tfStr[reg])
            oname[reg].close()
            print(outfile[reg] + " containing TF for DBBM has been created for region "+reg)

if __name__ == '__main__':
    args = parse_args()
    # Execution of the code begins here
    create_terraform_database_BM(args.inputfile, args.outdir, args.prefix, args.config)
