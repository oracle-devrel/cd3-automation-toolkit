...
def create_terraform_database_EXA(inputfile, outdir, prefix, config=DEFAULT_LOCATION):
    filename = inputfile
    ...
    for eachregion in uniquereg:
        ...
        srcdir = reg_out_dir + "/"
        resource = 'DBEXA'
        commonTools.backup_file(srcdir, resource, "EXA.tf")
    ...
    for reg in ct.all_regions:
        ...
        outfile[reg] = reg_out_dir + "/EXA.tf"

        if(tfStr[reg]!=''):
            oname[reg]=open(outfile[reg],'w')
            oname[reg].write(tfStr[reg])
            oname[reg].close()
            print(outfile[reg] + " containing TF for EXA has been created for region "+reg)

if __name__ == '__main__':
    args = parse_args()
    # Execution of the code begins here
    create_terraform_database_EXA(args.inputfile, args.outdir, args.prefix, args.config)
