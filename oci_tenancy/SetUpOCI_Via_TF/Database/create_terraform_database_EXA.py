...
# If the input is CD3
if ('.xls' in filename):
    df = pd.read_excel(filename, sheet_name='DB_System_EXA', skiprows=1)
    ...
    for i in df.index:
        ...
        database_backup_subnet_name = commonTools.check_tf_variable(database_backup_subnet_name.strip())
        ...
        resources_content = """
                resource "oci_database_db_system" \"""" + database_system_display_name_tf + """\" {
                  ...
                  backup_subnet_id    = "${oci_core_subnet.""" + database_backup_subnet_name + """.id}"
