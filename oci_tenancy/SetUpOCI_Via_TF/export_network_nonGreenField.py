...
def print_nsgsl(region, comp_name, vcn_name, nsg, nsgsl):
    ...
    print (region,comp_name,vcn_name,nsg.display_name, nsgsl.direction, nsgprotocol, nsgsl.is_stateless, nsgsourcetype, nsgsource, nsgdestinationtype,nsgdestination,sportmin,sportmax,dportmin,dportmax,icmptype,icmpcode,nsgsl.description)
    new_row=(region,comp_name,vcn_name,nsg.display_name, nsgsl.direction, nsgprotocol, nsgsl.is_stateless, nsgsourcetype, nsgsource, nsgdestinationtype, nsgdestination,sportmin,sportmax,dportmin,dportmax,icmptype,icmpcode,nsgsl.description)
    rows.append(new_row)
...
for reg in all_regions:
    ...
    for ntk_compartment_name in ntk_compartment_ids:
        ...
        for vcn in vcns.data:
            ...
            igw_display_name = "n"
            IGWs = oci.pagination.list_call_get_all_results(vnc.list_internet_gateways,compartment_id=ntk_compartment_ids[ntk_compartment_name],vcn_id=vcn.id,lifecycle_state="AVAILABLE")
            ...
            ngw_display_name = "n"
            NGWs = oci.pagination.list_call_get_all_results(vnc.list_nat_gateways,compartment_id=ntk_compartment_ids[ntk_compartment_name],vcn_id=vcn.id,lifecycle_state="AVAILABLE")
            ...
            sgw_display_name = "n"
            SGWs = oci.pagination.list_call_get_all_results(vnc.list_service_gateways,compartment_id=ntk_compartment_ids[ntk_compartment_name],vcn_id=vcn.id,lifecycle_state="AVAILABLE")
            ...
            lpg_display_names = ""
            LPGs = oci.pagination.list_call_get_all_results(vnc.list_local_peering_gateways,compartment_id=ntk_compartment_ids[ntk_compartment_name],vcn_id=vcn.id)
        ...
        for vcn in vcns.data:
            ...
            for ntk_compartment_name_again in ntk_compartment_ids:
                if (input_compartment_names is not None and ntk_compartment_name_again not in input_compartment_names):
                    continue
                NSGs = oci.pagination.list_call_get_all_results(vnc.list_network_security_groups,compartment_id=ntk_compartment_ids[ntk_compartment_name_again],vcn_id=vcn.id, lifecycle_state="AVAILABLE")
                nsglist = [""]
    ...
    for ntk_compartment_name in ntk_compartment_ids:
        ...
        for vcn in vcns.data:
            ...
            for ntk_compartment_name_again in ntk_compartment_ids:
                if (input_compartment_names is not None and ntk_compartment_name_again not in input_compartment_names):
                    continue
                DHCPs = oci.pagination.list_call_get_all_results(vnc.list_dhcp_options,compartment_id=ntk_compartment_ids[ntk_compartment_name_again],vcn_id=vcn.id, lifecycle_state="AVAILABLE")
    ...
    for ntk_compartment_name in ntk_compartment_ids:
        ...
        for vcn in vcns.data:
            ...
            for ntk_compartment_name_again in ntk_compartment_ids:
                ...
                Subnets = oci.pagination.list_call_get_all_results(vnc.list_subnets,compartment_id=ntk_compartment_ids[ntk_compartment_name_again],vcn_id=vcn.id, lifecycle_state="AVAILABLE")
                for subnet in Subnets.data:
                    ...
                    if(sl_names==""):
                        ...
                        sl_names = sl_names[:-1]
                    # Fill Subnets tab
                    print_subnets(region, ntk_compartment_name_again, vcn_info.display_name, subnet_info,dhcp_name,rt_name,sl_names,add_def_seclist)
