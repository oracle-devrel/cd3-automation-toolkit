...
If you want to just add the rules to existing route table or security list then dont use overwrite flag. It will read either the input csv containing
...
./modify_routes_tf.py <path to input csv containing rules> <path to outdir created earlier using create_all_tf_objects.py>
eg
./modify_routes_tf.py add-routes.csv /root/ocswork/terraform_files

Overwrite Mode:
./modify_routes_tf.py add-routes.csv /root/ocswork/terraform_files --overwrite yes
