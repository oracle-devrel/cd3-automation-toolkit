...
logFile=logging.FileHandler("/fss/logs/createSnapshot.log")
    ...
    for fss in all_fss.data:
        if (env == 'Dev'):
            if ('dev' in fss.display_name):
                logger.info(
                    'Creating snapshot for ' + fss.display_name + ' with name snaphot_' + fss.display_name + "_" + str(
                        datetime.date.today()))
                fss_id = fss.id
                createSnapshotDetails = oci.file_storage.models.CreateSnapshotDetails(file_system_id=fss_id,
                                                                                      name="snapshot_" + fss.display_name + "_" + str(
                                                                                          datetime.date.today()))
