...
######
def parse_args():
    parser = argparse.ArgumentParser(description="Create Network Sources terraform file")
    ...
    for i in df.index:
        ...
        for columnname in dfcolumns:
            ...
            if columnname == 'OCI Networks':
                if columnvalue != '' and columnvalue.strip().lower() != 'nan':
                    vcn_str = ""
                    VCNsAndIPRanges = columnvalue.split(";")

                    k = 0
                    while k < len(VCNsAndIPRanges):
                        VCNAndComp = VCNsAndIPRanges[k].split("=")[0]
                        compName = VCNAndComp.split("@")[0]
                        VCN = VCNAndComp.split("@")[1]
                        IPRanges = VCNsAndIPRanges[k].split("=")[1]

                        IPRange = IPRanges.split(",")

                        if "ocid" in VCN.strip():
                            vcn = VCN.strip()
                        else:
                            vcn = commonTools.check_columnvalue(VCN.strip())

                        count = 0
                        IPList= []
                        while count < len(IPRange):
                            IP = IPRange[count]
                            IPList.append(IP);
                            count += 1

                        vcn_IPRange = {}
                        vcn_IPRange["vcn_id"] = vcn
                        vcn_IPRange["ip_ranges"] = IPList
                        vcn_IPRange["CompartmentName"] = commonTools.check_tf_variable(compName)
                        VCNIPList.append(vcn_IPRange)

                        k += 1
                    tempdict = {'virtual_source_list': VCNIPList}
                    tempStr.update(tempdict)
                continue
...
if __name__ == '__main__':
    ...
    create_terraform_networkSources(args.inputfile, args.outdir,args.service_dir, args.prefix, config=args.config)