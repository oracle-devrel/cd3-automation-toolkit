...
module "vlans" {
  ...
  route_table_id   = each.value.route_table_name != null ? (length(regexall("ocid1.routeteable.oc1*", each.value.route_table_name)) > 0 ? each.value.route_table_name : data.oci_core_route_tables.oci_route_tables_vlans[each.key].route_tables.*.id[0]) : null
  #route_table_id    = each.value.route_table_name != null ? (length(regexall("ocid1.routeteable.oc1*", each.value.route_table_name)) > 0 ? each.value.route_table_name : try(data.oci_core_route_tables.oci_route_tables_vlans[each.key].route_tables.*.id[0], module.route-tables["${each.value.vcn_name}_${each.value.route_table_name}"]["route_table_ids"])): null
