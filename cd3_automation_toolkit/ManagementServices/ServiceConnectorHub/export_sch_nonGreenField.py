...
def get_service_connectors(region, SCH_LIST, sch_client, log_client, la_client, identity_client, stream_client,
                           notification_client, ct, values_for_column, ntk_compartment_name):
    volume_comp = ""
    log_source_list = []
    target_la_string = ""
    ...
    for schs in SCH_LIST.data:
        ...
        # Jira bug has been raised for fixing source details mismatch during tf apply.
        if target_kind == "functions" or target_kind == "monitoring" or source_kind == "monitoring":
            print(
                "Target kind functions/monitoring or Source Kind monitoring are not supported yet for export process...")
            pass
        else:
            if source_kind == "logging":
                ...
                for log in log_sources:
                    if log.log_group_id == "_Audit":
                        ...
                        log_source_list.append(f"{comp_name}&{log_group_name}")
...
def parse_args():
    ...
    parser.add_argument("--network-compartments", nargs='*', required=False,
                        help="comma seperated Compartments for which to export Service Connector Objects")
    return parser.parse_args()


def export_service_connectors(inputfile, _outdir, _config, network_compartments=[]):
    ...
    for reg in ct.all_regions:
        ...
        for ntk_compartment_name in comp_list_fetch:
            ...
            get_service_connectors(region, SCH_LIST, sch_client, log_client, la_client, identity_client,
                                   stream_client, notification_client, ct, values_for_column, ntk_compartment_name)
...
if __name__ == '__main__':
    ...
    export_service_connectors(args.inputfile, args.outdir, args.config, args.network_compartments)