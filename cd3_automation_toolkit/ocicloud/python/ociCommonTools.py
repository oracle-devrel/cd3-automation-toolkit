...
class ociCommonTools():
    tagColumns = {'freeform tags', 'freeform_tags', 'defined_tags', 'defined tags',
                  ...
                  'subscription defined tags', 'subscription_defined_tags', 'subscription freeform tags',
                  'subscription_freeform_tags'}

    drg_auto_RTs = {'Autogenerated Drg Route Table for RPC, VC, and IPSec attachments',
                    'Autogenerated Drg Route Table for VCN attachments'}
    drg_auto_RDs = {'Autogenerated Import Route Distribution for ALL routes',
    ...
    def get_subscribedregions(self, config, signer):
        ...
        for rs in regionsubscriptions.data:
            try:
                ...
                print(
                    "Skipping subscribed region " + rs.region_name + " . Check if domain is replicated for this region")
                continue
    ...
    def get_network_compartment_ids(self, c_id, c_name, config, signer):
        ...
        idc = IdentityClient(config=config, retry_strategy=oci.retry.DEFAULT_RETRY_STRATEGY, signer=signer)
        compartments = oci.pagination.list_call_get_all_results(idc.list_compartments, compartment_id=c_id,
    ...
    def get_compartment_map(self, var_file, resource_name):
        ...
        if resource_name in ntk_only_resources:
            ...
            print("\n")
            if resource_name in ["Compartments", "IAM Policies", "IAM Groups", "IAM Users", "Network Sources",
                                 "Tagging Objects"]:
                ...
                if self.fwl_clone_comp == "null":
                    ...
                    compartments = self.fwl_clone_comp if self.fwl_clone_comp else input(
                ...
                if self.vizoci_comp_filter == "null":
                    ...
                    compartments = self.vizoci_comp_filter if self.vizoci_comp_filter else input(
                        compartment_list_str.format(resource_name))
                input_compartment_names = list(
                    map(lambda x: x.strip(), compartments.split(','))) if compartments else None
                ...
                if self.comp_filter == "null":
                    ...
                    compartments = self.comp_filter if self.comp_filter else input(
                        compartment_list_str.format(resource_name))
                input_compartment_names = list(
                    map(lambda x: x.strip(), compartments.split(','))) if compartments else None
    ...
    def copy_file(src_dir, resource, pattern):
        dest_dir = str(src_dir) + "/backup_" + resource + "/" + datetime.datetime.now().strftime(
            "%d-%m-%H%M%S").replace('/', '-')
    ...
    def create_ingress_rule_string(self, secrule, tempStr, ingress_rule, tempdict2, region_seclist_name):
        ...
        tempdict2 = {'protocol_code': ociCommonTools.get_protocol(protocol), 'isstateless': isstateless}
        tempStr.update(tempdict2)
    ...
    def create_egress_rule_string(self, secrule, tempStr, egress_rule, tempdict2, region_seclist_name):
        ...
        if ("services-in-oracle-services-network" in tempStr['destination'] or "objectstorage" in tempStr[
            'destination']):
...
class parseSubnets():
    def __init__(self, filename):
        ...
        for i in df_subnet.index:
            ...
            if (region in commonTools.endNames):  # or str(region).lower()=='nan'):
                break
            key = df_subnet.loc[i, 'Region'].strip().lower(), df_subnet.loc[i, 'VCN Name'].strip() + "_" + \
                                                              df_subnet.loc[i, 'Display Name'].strip()
            value = df_subnet.loc[i, 'Compartment Name'].strip(), df_subnet.loc[i, 'VCN Name'].strip(), df_subnet.loc[
                i, 'Display Name'].strip()
