...
#from ocicloud.python.commonTools import *
import re
...
def validate_Firewall(filename,comp_ids,fwpolicy,config,signer, ct):
    ...
    dffirewall = commonTools.data_frame(filename, 'Firewall')
...
def validate_FirewallPolicy(filename, ct):
    ...
    dffwpolicy = commonTools.data_frame(filename, 'Firewall-Policy')
...
def validate_FirewallPolicyApplist(filename, fwpolicy_list,ct):
    ...
    dffwpolicyapplist = commonTools.data_frame(filename, 'Firewall-Policy-ApplicationList')
...
def validate_FirewallPolicyServicelist(filename, fwpolicy_list,ct):
    ...
    dffwpolicyservicelist = commonTools.data_frame(filename, 'Firewall-Policy-ServiceList')
...
def validate_FirewallPolicyUrllist(filename, fwpolicy_list,ct):
    ...
    dffwpolicyUrllist = commonTools.data_frame(filename, 'Firewall-Policy-UrlList')
...
def validate_FirewallPolicyAddress(filename, fwpolicy_list,ct):
    ...
    dffwpolicyAddress = commonTools.data_frame(filename, 'Firewall-Policy-AddressList')
...
def validate_FirewallPolicySecrets(filename, fwpolicy_list, config,signer, ct):
    ...
    dffwpolicysecret = commonTools.data_frame(filename, 'Firewall-Policy-Secret')
...
def validate_FirewallPolicyDecryption(filename, fwpolicy_list,ct):
    ...
    dffwpolicydecrypt = commonTools.data_frame(filename, 'Firewall-Policy-DecryptProfile')
...
def validate_FirewallPolicyDecryptionRule(filename, fwpolicy_list, fulladdreslist, fullsercretslist, fulldecryptlist,ct):
    ...
    dffwpolicydecryptrule = commonTools.data_frame(filename, 'Firewall-Policy-DecryptRule')
    dfcolumns = dffwpolicydecryptrule.columns.values.tolist()

    dffwdecryptrule = commonTools.data_frame(filename, 'Firewall-Policy-DecryptRule')
...
def validate_FirewallPolicyTunnelInspectRule(filename, fwpolicy_list, fulladdreslist, ct):
    ...
    dffwpolicytunnelinspectrule = commonTools.data_frame(filename, 'Firewall-Policy-TunnelInspect')
    dfcolumns = dffwpolicytunnelinspectrule.columns.values.tolist()

    dffwtunnleinspectrule = commonTools.data_frame(filename, 'Firewall-Policy-TunnelInspect')
    ...
    if any([fwpolicytunnelinspectrule_empty_check, fwpolicytunnelinspectrule_invalid_check,
        ...
        return False
def validate_FirewallPolicyNatRule(filename, fwpolicy_list, fulladdreslist, fullnatservicelist,ct):
    fwpolicynatrule_empty_check = False
    fwpolicynatrule_invalid_check = False
    fwpolicynatrule_check = []
    fwpolicynatrule_nameg_length = False
    fwpolicynatrulesa_check = []
    fwpolicynatruleda_check = []
    fwpolicynatrulepost_check = []

    dffwpolicynatrule = commonTools.data_frame(filename, 'Firewall-Policy-NatRules')
    dfcolumns = dffwpolicynatrule.columns.values.tolist()

    dffwnatrule = commonTools.data_frame(filename, 'Firewall-Policy-NatRules')
    dffwnatrule_list = dffwnatrule['Rule Name'].astype(str)
    dffwnatrulepolicy_list = dffwnatrule['Firewall Policy'].astype(str)
    fullnatrulelist = dffwnatrulepolicy_list + '::' + dffwnatrule_list

    for i in dffwpolicynatrule.index:
        region = str(dffwpolicynatrule.loc[i, 'Region']).strip().lower()
        # Encountered <End>
        if (region in commonTools.endNames):
            break
        if region == 'nan':
            log(f'ROW {i + 3} : Empty value at column "Region".')
            fwpolicynatrule_empty_check = True
        elif region not in ct.all_regions:
            log(f'ROW {i + 3} : "Region" {region} is not subscribed for tenancy.')
            fwpolicynatrule_invalid_check = True
        for columnname in dfcolumns:
            # Column value
            columnvalue = str(dffwpolicynatrule.loc[i, columnname]).strip()
            if (columnname == 'Firewall Policy'):
                if columnvalue.lower() == 'nan':
                    log(f'ROW {i + 3} : Empty value at column Policy Name.')
                    fwpolicynatrule_empty_check = True
            ...
            if (columnname == 'Rule Name'):
                ...
                if columnvalue.lower() != 'nan':
                    if (len(columnvalue) > 63) or (len(columnvalue) < 2):
                        log(f'ROW {i + 3} : Nat rule Name "{columnvalue}" has more alphanumeric characters than the allowed maximum limit of 63.')
                        fwpolicynatrule_nameg_length = True
                    if (validate_names(columnvalue) == True):
                        log(f'ROW {i + 3} : Only alphabets, digits, - and _ are allowed in the Tunnel inspection Rule Name')
                        fwpolicynatrule_invalid_check == True
            if (columnname == 'Source Address'):
                if columnvalue.lower() != 'nan':
                    sa_list = columnvalue.split(",")
                    for eachsa in sa_list:
                        fwpolicyname = str(dffwpolicynatrule.loc[i, 'Firewall Policy']).strip()
                        finalsalist = fwpolicyname + '::' + eachsa
                        fwpolicynatrulesa_check.append(compare_values(fulladdreslist.tolist(), finalsalist,[i, 'Source Address','Firewall-Policy-Address','Address list']))
            if (columnname == 'Destination Address'):
                if columnvalue.lower() != 'nan':
                    da_list = columnvalue.split(",")
                    for eachda in da_list:
                        fwpolicyname = str(dffwpolicynatrule.loc[i, 'Firewall Policy']).strip()
                        finaldalist = fwpolicyname + '::' + eachda
                        fwpolicynatruleda_check.append(compare_values(fulladdreslist.tolist(), finaldalist,[i, 'Destination Address','Firewall-Policy-Address','Address list']))
            if (columnname == 'Service'):
                if columnvalue.lower() != 'nan':
                    finalservice = []
                    fwpolicyname = str(dffwpolicynatrule.loc[i, 'Firewall Policy']).strip()
                    finalservicelist = fwpolicyname + '&' + columnvalue
                    natserv = fullnatservicelist.tolist()
                    for service1 in  natserv:
                        spolicy,snst = service1.split("&")
                        if snst is not "nan" and "\n" in snst:
                            tempservices = snst.split("\n")
                            for intservice in tempservices:
                                finalservice_value = spolicy + "&" +  intservice.split("::")[0]
                                finalservice.append(finalservice_value)
                        else:
                            finalservice_value = spolicy + "&" + snst.split("::")[0]
                            finalservice.append(finalservice_value)
                    fwpolicynatruleda_check.append(compare_values(finalservice, finalservicelist, [i, 'Service', 'Firewall-Policy-ServiceList', 'Services']))
            if (columnname == 'Action'):
                if (columnvalue not in ['DIPP_SRC_NAT', 'Dipp_Src_Nat', 'Dipp_src_nat', 'dipp_src_nat']):
                    log(f'ROW {i + 3} : Action "{columnvalue}" is not a valid option, it should be either DIPP_SRC_NAT/Dipp_Src_Nat/Dipp_src_nat/dipp_src_nat.')
                    fwpolicynatrule_invalid_check = True
            if (columnname == 'Type'):
                if (columnvalue not in ['NATV4', 'Natv4', 'natv4', 'NatV4']):
                    log(f'ROW {i + 3} : Action "{columnvalue}" is not a valid option, it should be either NATV4/Natv4/NatV4/natv4.')
                    fwpolicynatrule_invalid_check = True
                ...
                if columnvalue.lower() != 'nan':
                    ...
                    if len(post) != 2:
                        ...
                        if post[1].lower() != 'nan':
                            fwpolicyname = str(dffwpolicynatrule.loc[i, 'Firewall Policy']).strip()
                            finalrulepost = fwpolicyname + '::' + post[1]
                            fwpolicynatrulepost_check.append(
                                compare_values(fullnatrulelist.tolist(), finalrulepost,[i, 'Position', 'Firewall-Policy-TunnelInspect', 'Rule name']))

    if any([fwpolicynatrule_empty_check, fwpolicynatrule_invalid_check,
            fwpolicynatrule_nameg_length]) or any(fwpolicynatrule_check) or any(
            fwpolicynatrulesa_check) or any(fwpolicynatruleda_check) or  any(
            fwpolicynatrulepost_check):
        print("Null or Wrong value Check failed!!")
        return True
    else:
...
def validate_FirewallPolicySecurityRule(filename, fwpolicy_list, fulladdreslist, fullservicelist, fullappslist, fullurlslist,ct):
    ...
    dffwpolicysecurityrule = commonTools.data_frame(filename, 'Firewall-Policy-SecRule')
    dfcolumns = dffwpolicysecurityrule.columns.values.tolist()

    dffwsecurityrule = commonTools.data_frame(filename, 'Firewall-Policy-SecRule')
...
def validate_compartments(filename,ct):
    ...
    dfcomp = commonTools.data_frame(filename ,'Compartments')
...
def validate_firewall_cd3(filename, var_file, prefix, outdir, config,signer,ct):
    ...
    dffwaddress = commonTools.data_frame(filename, 'Firewall-Policy-AddressList')
    ...
    dffwservice = commonTools.data_frame(filename, 'Firewall-Policy-ServiceList')
    dffwservice_list = dffwservice['Service List'].astype(str)
    dffwnatservice_list = dffwservice['Services'].astype(str)
    #spdffwnatservice_list = dffwnatservice_list.str.split(pat="::",n=1).astype(str)
    dffwservicepolicy_list = dffwservice['Firewall Policy'].astype(str)
    fullservicelist = dffwservicepolicy_list + '::' + dffwservice_list
    fullnatservicelist = dffwservicepolicy_list + '&' + dffwnatservice_list

    dffwapps = commonTools.data_frame(filename, 'Firewall-Policy-ApplicationList')
    ...
    dffwurls = commonTools.data_frame(filename, 'Firewall-Policy-UrlList')
    ...
    dffwsecrets = commonTools.data_frame(filename, 'Firewall-Policy-Secret')
    ...
    dffwdecrypt = commonTools.data_frame(filename, 'Firewall-Policy-DecryptProfile')
    ...
    fw_policynatrules_check = validate_FirewallPolicyNatRule(filename, fwpolicy_list, fulladdreslist,fullnatservicelist, ct)
    # Prints the final result; once the validation is complete
    if any([Firewall_check, fw_policy_check, fw_policyapp_check, fw_policyurl_check, fw_policyservice_check, fw_policyaddress_check, fw_policysecrets_check, fw_policydecryption_check, fw_policydecryptionrule_check, fw_policysecurityrule_check, fw_policytunnelinspect_check, fw_policynatrules_check]):
        log("=======")
        log("Summary:")
        log("=======")
