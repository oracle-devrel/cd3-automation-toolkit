                        ...
                        for comp_id in dict.fromkeys(comp_id_list):
                            ...
                            for key in keys.data:
                                ...
                                if key.lifecycle_state not in ["DELETED", "PENDING_DELETION", "SCHEDULING_DELETION"]:
                                    ...
                                    if get_key_data.vault_id == vault.id:
                                        if first_key:
                                                ...
                                                if col_header == 'Region':
                                                    ...
                                                    if not replicas:
                                                        ...
                                                        if region_name:
                                                            ...
                                                            values_for_column_kms[col_header].append('')
                                                elif str(col_header).lower() in ["vault defined tags",
                                                                                 "vault freeform tags"]:
                                                    values_for_column_kms = ociCommonTools.export_tags(vault,
                                                                                                       col_header,
                                                                                                       values_for_column_kms)

                                                elif col_header == 'Key Compartment Name':
                                                    comp_name = comp_name_list[
                                                        comp_id_list.index(get_key_data.compartment_id)]
                                                    ...
                                                    values_for_column_kms[col_header].append(
                                                        get_key_data.protection_mode)
                                                elif col_header == 'Algorithm':
                                                    values_for_column_kms[col_header].append(
                                                        get_key_data.key_shape.algorithm)
                                                elif col_header == 'Length in bits':
                                                    values_for_column_kms[col_header].append(
                                                        (get_key_data.key_shape.length) * 8)
                                                    ...
                                                    values_for_column_kms[col_header].append(
                                                        "TRUE" if get_key_data.is_auto_rotation_enabled == True else "FALSE")
                                                elif col_header == 'Rotation interval in days':
                                                    values_for_column_kms[col_header].append(
                                                        get_key_data.auto_key_rotation_details.rotation_interval_in_days if hasattr(
                                                            get_key_data.auto_key_rotation_details,
                                                            'rotation_interval_in_days') else '')
                                                elif str(col_header).lower() in ["key defined tags",
                                                                                 "key freeform tags"]:
                                                    if len(key.defined_tags) != 0:
                                                        values_for_column_kms = ociCommonTools.export_tags(key,
                                                                                                           col_header,
                                                                                                           values_for_column_kms)
                                                    else:
                                                        values_for_column_kms[col_header].append('')
                                                else:
                                                    oci_objs = [vault, key, get_key_data, get_vault_data,
                                                                get_vault_data.replica_details]
                                                    values_for_column_kms = ociCommonTools.export_extra_columns(
                                                        oci_objs, col_header,
                                                        sheet_dict_kms,
                                                        values_for_column_kms)
                        ...
                        if not keys_found_for_vault:
                            ...
                            for col_header in values_for_column_kms.keys():
                                ...
                                if col_header == 'Region':
                                    ...
                                    values_for_column_kms[col_header].append(region_name if region_name else '')
