...
def fwpolicy_create_natrules(inputfile, outdir, service_dir, prefix, ct):
    # Load the template file
    file_loader = FileSystemLoader(f'{Path(__file__).parent}/templates')
    env = Environment(loader=file_loader, keep_trailing_newline=True)
    natrules = env.get_template('policy-natrules-template')


    sheetName = "Firewall-Policy-NatRules"
    natrules_auto_tfvars_filename = prefix + "_"+sheetName.lower()+".auto.tfvars"
    ...
    for reg in ct.all_regions:
        ...
        commonTools.backup_file(reg_out_dir, resource, natrules_auto_tfvars_filename)
    ...
    for i in df.index:
        ...
        natrules_str[region] = natrules_str[region] + natrules.render(tempStr)

    for reg in region_list:
        reg_out_dir = outdir + "/" + reg + "/" + service_dir
        if not os.path.exists(reg_out_dir):
            os.makedirs(reg_out_dir)
        outfile[reg] = reg_out_dir + "/" + natrules_auto_tfvars_filename
        if natrules_str[reg] != '':
            # Generate Final String
            src = "##Add New Nat rules for " + reg.lower() + " here##"
            natrules_str[reg] = natrules.render(count=0, region=reg).replace(src, natrules_str[reg] + "\n" + src)
            natrules_str[reg] = "".join([s for s in natrules_str[reg].strip().splitlines(True) if s.strip("\r\n").strip()])
            natrules_str[reg] = "\n\n" + natrules_str[reg]
            oname[reg] = open(outfile[reg], 'a')
            oname[reg].write(natrules_str[reg])
            oname[reg].close()
            print(outfile[reg] + " containing TF for Firewall Policy Nat rules has been updated for region " + reg)
