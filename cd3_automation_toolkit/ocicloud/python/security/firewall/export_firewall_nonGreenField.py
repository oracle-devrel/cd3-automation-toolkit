...
def print_firewall(region, export_tags, ct, values_for_column_fw, fws, fw_compartment_name, vcn, fw,state):
    for eachfw in fws.data:
        ...
        #if eachfw.nat_configuration ==  null:

        enable_private_nat =""
        if eachfw.nat_configuration is not None:
            enable_private_nat = eachfw.nat_configuration.must_enable_private_nat
        # Fetch policy
        ...
        for col_header in values_for_column_fw:
            if col_header == 'Region':
            ...
            elif col_header == 'Enable Private NAT(True/False)':
                values_for_column_fw[col_header].append(enable_private_nat)
                ...
                values_for_column_fw = ociCommonTools.export_tags(eachfw, col_header, values_for_column_fw)
