...
importCommands,importCommands_nfp,importCommands_nfao,importCommands_ulo,importCommands_slo,importCommands_alo,importCommands_sro,importCommands_mso,importCommands_dpo,importCommands_dro,importCommands_fpo,importCommands_tio,importCommands_nato = {},{},{},{},{},{},{},{},{},{},{},{},{}
oci_obj_names = {}
    ...
    for applistpolicy in fwpolicies:
            ...
            for col_header in values_for_column_fwapplist:
                if col_header == 'Region':
                    ...
                    values_for_column_fwapplist = ociCommonTools.export_tags(applistpolicy, col_header,
            ...
            for rules in secrules_info:
                ...
                for col_header in values_for_column_fwsecrules:
                    if col_header == 'Region':
                        ...
                        values_for_column_fwsecrules = ociCommonTools.export_tags(secrulespolicy, col_header,values_for_column_fwsecrules)
            ...
            for drules in decrypteules_info:
                ...
                for col_header in values_for_column_fwdecryptrule:
                    if col_header == 'Region':
                        ...
                        values_for_column_fwdecryptrule = ociCommonTools.export_tags(decryptrulepolicy, col_header,values_for_column_fwdecryptrule)
            ...
            for tirules in tunnelinspectrule_info:
                ...
                for col_header in values_for_column_fwtunnelinspect:
                    if col_header == 'Region':
                        ...
                        values_for_column_fwtunnelinspect = ociCommonTools.export_tags(tunnelinspectrulepolicy, col_header,values_for_column_fwtunnelinspect)
    return values_for_column_fwtunnelinspect


def print_firewall_nat(region, ct, values_for_column_fwnat, fwpolicies, fwclient, export_tags, state):
    if not clone:
        print("Exporting Nat rules details " + region)
    for natrulepolicy in fwpolicies:

        # Tags filter
        defined_tags = natrulepolicy.defined_tags
        tags_list = []
        if defined_tags:
            for tkey, tval in defined_tags.items():
                for kk, vv in tval.items():
        ...
        natrulepolicy_id = natrulepolicy.id
        natrulepolicy_display_name = natrulepolicy.display_name
        if clone:
            natrulepolicy_display_name = target_pol[src_pol.index(natrulepolicy_display_name)]
        natrulepolicy_tf_name = commonTools.check_tf_variable(natrulepolicy_display_name)
        fwnatrules = oci.pagination.list_call_get_all_results(fwclient.list_nat_rules, natrulepolicy_id)
        natrule_info = fwnatrules.data
        for nirules in natrule_info:
            nirule_info = fwclient.get_nat_rule(nirules.parent_resource_id, nirules.name).data
            nirules_display_name = nirules.name
            nirules_tf_name = commonTools.check_tf_variable(nirules_display_name)
            tf_resource = f'module.nat_rules[\\"{str(natrulepolicy_tf_name)}_{str(nirules_tf_name)}\\"].oci_network_firewall_network_firewall_policy_nat_rule.network_firewall_policy_nat_rule'
            if not clone and tf_resource not in state["resources"]:
                importCommands_nato[reg] += f'\n{tf_or_tofu} import "{tf_resource}" networkFirewallPolicies/{natrulepolicy_id}/natRules/{nirules_display_name}'

            rsrc_detail = ""
            rdst_detail = ""
            if nirule_info.condition.source_address != None:
                for rsrc in nirule_info.condition.source_address:
                    rsrc_detail = rsrc_detail + "," + rsrc
                if (rsrc_detail != ""):
                    rsrc_detail = rsrc_detail[1:]
            if nirule_info.condition.destination_address != None:
                for rdst in nirule_info.condition.destination_address:
                    rdst_detail = rdst_detail + "," + rdst
                if (rdst_detail != ""):
                    rdst_detail = rdst_detail[1:]

            if nirule_info.position.after_rule == None and nirule_info.position.before_rule == None:
                dposition = None
            elif nirule_info.position.after_rule == None:
                dposition = None
            elif nirule_info.position.before_rule == None:
                dposition = "after_rule::" + nirule_info.position.after_rule
            else:
                dposition = "after_rule::" + nirule_info.position.after_rule

            for col_header in values_for_column_fwnat:
                if col_header == 'Region':
                    values_for_column_fwnat[col_header].append(region)
                elif col_header == 'Firewall Policy':
                    values_for_column_fwnat[col_header].append(natrulepolicy_display_name)
                elif col_header == 'Rule Name':
                    values_for_column_fwnat[col_header].append(nirules_display_name)
                elif col_header == 'Source Address':
                    values_for_column_fwnat[col_header].append(rsrc_detail)
                elif col_header == 'Destination Address':
                    values_for_column_fwnat[col_header].append(rdst_detail)
                elif col_header == 'Action':
                    values_for_column_fwnat[col_header].append(nirule_info.action)
                elif col_header == 'Type':
                    values_for_column_fwnat[col_header].append(nirule_info.type)
                elif col_header == 'Service':
                    values_for_column_fwnat[col_header].append(nirule_info.condition.service)
                elif col_header == 'Description':
                    values_for_column_fwnat[col_header].append(nirule_info.description)
                elif col_header == 'Position':
                    values_for_column_fwnat[col_header].append(dposition)
                elif col_header.lower() in ociCommonTools.tagColumns:
                    values_for_column_fwnat = ociCommonTools.export_tags(natrulepolicy, col_header,
                                                                      values_for_column_fwnat)
    return values_for_column_fwnat
...
def export_firewallpolicy(inputfile, _outdir, service_dir, config, signer, ct, export_compartments, export_regions=[], export_tags=[], export_policies=[],target_policies=[],attached_policy_only="",clone_policy=False):
    ...
    global sheet_dict
    global importCommands,importCommands_nfp,importCommands_nfao,importCommands_ulo,importCommands_slo,importCommands_alo,importCommands_sro,importCommands_mso,importCommands_dpo,importCommands_dro,importCommands_fpo,importCommands_tio,importCommands_nato
    ...
    df, values_for_column_fwnat = commonTools.read_cd3(cd3file, "Firewall-Policy-NatRules")
    ...
    if not clone:
        ...
        for reg in export_regions:
            ...
            importCommands_dpo[reg], importCommands_dro[reg], importCommands_fpo[reg], importCommands_tio[reg], importCommands_nato[reg] = "", "", "", "", "", "", "", "", "", "", "", "",""
    ...
    for reg in export_regions:
        ...
        for compartment_name in export_compartments:
            ...
            values_for_column_fwnat = print_firewall_nat(region, ct, values_for_column_fwnat,fwpolicies, fwclient, export_tags, state)
    if clone:
        ...
        commonTools.write_to_cd3(values_for_column_fwnat, cd3file, "Firewall-Policy-NatRules", append=True)
    else:
        ...
        commonTools.write_to_cd3(values_for_column_fwnat, cd3file, "Firewall-Policy-NatRules")
        print("Firewall Policies exported to CD3\n")

        # writing data
        init_commands = f'\n######### Writing import for Network firewall policy Objects #########\n\n#!/bin/bash\n{tf_or_tofu} init'
        importCommands_message = ["Policy","Address Objects","url list Objects","service list Objects","application list Objects","Security Rules Objects","Mapped Secret Objects","Decrypt profile Objects","decryption Rules Objects","policy Objects","Tunnel Inspect Objects","Nat Rules Objects"]
        for reg in export_regions:
            count = 0
            all_importCommands = [importCommands_nfp[reg], importCommands_nfao[reg], importCommands_ulo[reg], importCommands_slo[reg],
                                  ...
                                  importCommands_dro[reg], importCommands_fpo[reg], importCommands_tio[reg], importCommands_nato[reg]]
