...
def print_oke(values_for_column_oke, reg, compartment_name, compartment_name_nodepool,nodepool_count, nodepool_info,cluster_info,network,nodepool_type,ct):
    ...
    for col_header in values_for_column_oke.keys():
        ...
        elif 'oke labels' in col_header.lower() and col_header.lower() in ociCommonTools.tagColumns:
            if (nodepool_info != None):
                values_for_column_oke = ociCommonTools.export_tags(nodepool_info, col_header,values_for_column_oke)
            else:
                values_for_column_oke[col_header].append(None)

        elif 'nodepool defined tags' in col_header.lower() and col_header.lower() in ociCommonTools.tagColumns:
            if (nodepool_info != None):
                values_for_column_oke = ociCommonTools.export_tags(nodepool_info, col_header,values_for_column_oke)
            else:
                values_for_column_oke[col_header].append(None)
        ...
        elif 'node defined tags' in col_header.lower() and col_header.lower() in ociCommonTools.tagColumns:
            if (nodepool_info != None):
                if nodepool_type=='managed':
                    values_for_column_oke = ociCommonTools.export_tags(nodepool_info.node_config_details, col_header,values_for_column_oke)
                elif (nodepool_type=='virtual' and nodepool_info.virtual_node_tags!=None):
                    values_for_column_oke = ociCommonTools.export_tags(nodepool_info.virtual_node_tags , col_header,
        ...
        elif 'node freeform tags' in col_header.lower() and col_header.lower() in ociCommonTools.tagColumns:
            if (nodepool_info != None):
                if nodepool_type == 'managed':
                    values_for_column_oke = ociCommonTools.export_tags(nodepool_info.node_config_details, col_header,values_for_column_oke)
                elif (nodepool_type == 'virtual' and nodepool_info.virtual_node_tags!=None):
        ...
        elif 'volume freeform tags' in col_header.lower() and col_header.lower() in ociCommonTools.tagColumns:
            if (nodepool_count <= 1):
