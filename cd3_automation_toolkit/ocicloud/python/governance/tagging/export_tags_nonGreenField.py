...
def  print_tags(values_for_column_tags,region, ntk_compartment_name, tag, tag_key, tag_default_value,reg,state):
    ...
    for col_header in values_for_column_tags.keys():
        if (col_header == "Region"):
            ...
            values_for_column_tags = ociCommonTools.export_tags(tag, col_header, values_for_column_tags)
        else:
            oci_objs = [tag,tag_key]#,tag_default]
            values_for_column_tags = ociCommonTools.export_extra_columns(oci_objs, col_header, sheet_dict_tags,values_for_column_tags)
