...
def create_terraform_notifications(inputfile, outdir, service_dir, prefix, ct):
    ...
    for i in df.index:
        ...
        tempdict = {'subscriptions': subscription}
        tempStr.update(tempdict)

        topic_key = f'{region}_{topic}'

        if topic_key in topic_dict:
            topic_dict[topic_key]['subscriptions'].extend(subscription)
        else:
            topic_dict[topic_key] = deepcopy(tempStr)

    for key in topic_dict:
        reg_key = key.split("_")[0]

        # Write all info to TF string
        tfStr[reg_key]=tfStr[reg_key][:-1]  + notifications_template.render(topic_dict[key])
