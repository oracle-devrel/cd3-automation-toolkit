    ...
    for col_header in values_for_column_fss.keys():
        if (col_header == "Region"):
            ...
            if len(allowed_auth) == 1:
                ...
                values_for_column_fss[col_header].append('')
        elif str(col_header).lower() in ociCommonTools.tagColumns:
            values_for_column_fss = ociCommonTools.export_tags(fsinfo.data, col_header, values_for_column_fss)
        else:
            oci_objs = [fsinfo.data, mnt_info1,einfo_path]
            values_for_column_fss = ociCommonTools.export_extra_columns(oci_objs, col_header, sheet_dict_instances,
