...
#
# This script will produce a Terraform file that will be used to set up OCI CDB
# Database EXA CDB
#
# Author: Suruchi
# Oracle Consulting
...
def create_terraform_exa_cdb(inputfile, outdir, service_dir, prefix, ct):
    filename = inputfile
    sheetName = "EXA-CDBs"
    ...
    template = env.get_template('exa-cdb-template')
    ...
    for i in df.index:
        ...
        for field in mandatory_base_fields + mandatory_cdb_fields:
            if field not in dfcolumns or str(df.loc[i, field]).strip().lower() in ['nan', '']:
                print(
                    f"\nWARNING!!! Skipping row {i + 2} in sheet '{sheetName}' "
        ...
        for columnname in dfcolumns:
            ...
            if columnname == "CDB Name":
                ...
                tempdict = {'db_name_tf_name': display_tf_name}


            if columnname == "Default PDB Name":
                display_tf_name = columnvalue.strip()
                #display_tf_name = commonTools.check_tf_variable(display_tf_name)
                tempdict = {'default_pdb_name_tf_name': display_tf_name}

            if columnname == "Backup Dest Type":
                display_tf_name = columnvalue.strip()
                if display_tf_name:
                    tempdict = {'backup_dest_type_tf_name': display_tf_name}
                else:
                    tempdict = {'backup_dest_type_tf_name': None}

            if columnname == "Admin Password":
                display_tf_name = columnvalue.strip()
                if display_tf_name.startswith("ocid1.vaultsecret."):
                    tempdict = { 'admin_password' : None,
                                 'admin_secret_id' : display_tf_name }
                else:
                    tempdict = { 'admin_password' : display_tf_name,
    ...
    for reg in ct.all_regions:
        ...
        if (tfStr[reg] != ''):
            ...
            marker = f"##Add New CDBs for {reg.lower()} here##"
            ...
            print(outfile[reg] + " for EXA CDBs has been created for region " + reg)
