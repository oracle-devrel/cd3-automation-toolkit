    ...
    for seclist in seclists.data:
        ...
        for rule in isec_rules:
            ...
            if rule.protocol == "6":
                ...
                if rule.icmp_options is None:
                    printstr= (dn + ",ingress,ipv6-icmp," + str(rule.is_stateless) + "," + rule.source + ",,,,,,,,"+desc)
                    insert_values(values_for_column,oci_objs,region, comp_name, vcn_name, 'ingress', 'ipv6-icmp','', '', '','', '', '')
                else:
                    code = convertNullToNothing(rule.icmp_options.code)
                    type = convertNullToNothing(rule.icmp_options.type)
                    printstr= (dn + ",ingress,ipv6-icmp," + str(rule.is_stateless) + "," + rule.source + ",,,,,," + type + "," + code+","+desc)
                    insert_values(values_for_column,oci_objs,region, comp_name, vcn_name, 'ingress', 'ipv6-icmp', '', '','', '', type, code)
...
def export_seclist(inputfile, outdir, service_dir,config,signer, ct, export_compartments,export_regions,export_tags,_tf_import_cmd):
    ...
    for reg in export_regions:
        config.__setitem__("region", ociCommonTools().region_dict[reg])
