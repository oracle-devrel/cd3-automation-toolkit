...
def print_nsgsl(values_for_column_nsgs,vnc,region, comp_name, vcn_name, nsg, nsgsl,i,state):
    ...
    if nsgsl.protocol.lower()!="all":
        protocol = str(ociCommonTools().protocol_dict[nsgsl.protocol].lower())
    else:
        protocol="all"
    for col_header in values_for_column_nsgs.keys():
        if (col_header == "Region"):
            ...
            values_for_column_nsgs = ociCommonTools.export_tags(nsg, col_header, values_for_column_nsgs)
        else:
            oci_objs = [nsg,nsgsl]
            values_for_column_nsgs = ociCommonTools.export_extra_columns(oci_objs, col_header, sheet_dict_nsgs,values_for_column_nsgs)
