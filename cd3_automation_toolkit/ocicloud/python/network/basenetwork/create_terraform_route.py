...
def create_terraform_drg_route(inputfile, outdir, service_dir, prefix, ct, non_gf_tenancy,network_connectivity_in_setupoci, modify_network):
    ...
    if ('.xls' in filename):
        ...
        #            continue
            ...
            if (DRG_RT in ociCommonTools.drg_auto_RTs and DRG_RD in ociCommonTools.drg_auto_RDs):
                continue
            ...
            if (DRG_RT != 'nan' and DRG_RT not in ociCommonTools.drg_auto_RTs and region_rt_name not in common_rt):
                    drg_rt[region] = drg_rt[region] + drg_rt_template.render(tempStr)

            if (DRG_RD.lower() != 'nan' and DRG_RD not in ociCommonTools.drg_auto_RDs):
