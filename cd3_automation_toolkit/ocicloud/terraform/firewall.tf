...
module "firewalls" {
  ...
  must_enable_private_nat    = each.value.must_enable_private_nat
...
module "secrets" {
  ...
  vault_secret_id            = length(regexall("ocid1.vaultsecret.oc*", each.value.vault_secret_id)) > 0 ? each.value.vault_secret_id : data.oci_vault_secrets.fw_secret[each.key].secrets.*.id[0]
...
module "nat_rules" {
  source                     = "./modules/security/firewall/nat-rules"
  for_each                   = var.nat_rules != null ? var.nat_rules : {}
  depends_on                 = [module.policies, module.address_lists, module.service_lists]
  action                     = each.value.action
  rule_name                  = each.value.rule_name
