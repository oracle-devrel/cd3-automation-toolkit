...
resource "oci_containerengine_virtual_node_pool" "virtual_nodepool" {
  ...
  dynamic "placement_configurations" {
    ...
    content {
      availability_domain = data.oci_identity_availability_domains.ads.availability_domains[placement_configurations.value.availability_domain].name
      subnet_id           = placement_configurations.value.subnet_id
      fault_domain        = placement_configurations.value.fault_domain
    }
  }
