...
resource "oci_sch_service_connector" "service_connector" {
  ...
  source {
    ...
    dynamic "log_sources" {
      for_each = local.indexed_map #{for idx,val in local.source_log_group_detail:idx => val}
      content {
        compartment_id = log_sources.value.compartment_id #split("@", log_sources.key)[0]
        log_group_id   = log_sources.value.log_group_id   #length(regexall("Audit", split("@", log_sources.key)[1])) > 0 ? (length(regexall("Audit_In_Subcompartment", split("@", log_sources.key)[1])) > 0 ? "_Audit_Include_Subcompartment" : "_Audit") : data.oci_logging_log_groups.source_log_groups[log_sources.key].log_groups[0].id
        log_id         = log_sources.value.log_id         #lower(split("@", log_sources.key)[2]) == "all" ? null : data.oci_logging_logs.source_logs[log_sources.key].logs[0].id
      }
    }
