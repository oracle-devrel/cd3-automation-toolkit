...
module "alarms" {
  ...
  destinations                 = [for tn in each.value.destinations : (length(regexall("ocid1.onstopic.oc*", tn)) > 0 ? tn : merge(module.notifications.*...)[tn]["topic_tf_id"])]
...
module "events" {
  ...
  actions        = var.events
  key_name       = each.key
  topic_name     = merge(module.notifications.*...)
...
module "notifications" {
  source   = "./modules/managementservices/notification"
